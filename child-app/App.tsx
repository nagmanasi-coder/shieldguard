...
import { manualSync, getSyncStats, type SyncStats } from './src/services/SmartSyncService'
...
export default function App() {
  ...
  async function startBackgroundMonitoring() {
    try {
      ...
      const interval = setInterval(() => { setSyncStats(getSyncStats()) }, 30000)
      return () => clearInterval(interval)
    } catch (err) {
      console.error('Background monitoring failed:', err)
    }
  }
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.efficiencyCard}>
          ...
          <View style={styles.effRow}>
            <View style={styles.effItem}>
              <Text style={styles.effNumber}>{syncStats.locationPings}</Text>
              <Text style={styles.effLabel}>Location Pings</Text>
            </View>
            <View style={styles.effItem}>
              <Text style={styles.effNumber}>{syncStats.fullSyncs}</Text>
              <Text style={styles.effLabel}>Full Syncs</Text>
            </View>
            <View style={styles.effItem}>
              <Text style={styles.effNumber}>{syncStats.screenshots}</Text>
              <Text style={styles.effLabel}>Screenshots</Text>
            </View>
            <View style={styles.effItem}>
              <Text style={[styles.effNumber, { color: '#10b981' }]}>{syncStats.writesSaved}</Text>
              <Text style={styles.effLabel}>Writes Saved</Text>
            </View>
          </View>
          {syncStats.lastSyncReason ? (
            <View style={styles.lastSyncRow}>
              <Ionicons name="time-outline" size={14} color="#64748b" />
              <Text style={styles.lastSyncText}>Last: {syncStats.lastSyncReason}</Text>
            </View>
          ) : null}
        </View>
        ...
        <View style={styles.monitoringCard}>
          ...
          {[
            { icon: 'location-outline' as const, color: '#10b981', text: 'Location', interval: 'Always' },
            { icon: 'camera-outline' as const, color: '#3b82f6', text: 'Screenshot on Unlock', interval: 'On unlock' },
            { icon: 'chatbubble-outline' as const, color: '#8b5cf6', text: 'SMS Monitoring', interval: 'On unlock' },
            { icon: 'call-outline' as const, color: '#f59e0b', text: 'Call Log', interval: 'On unlock' },
            { icon: 'shield-outline' as const, color: '#06b6d4', text: 'Threat Detection', interval: 'On unlock' },
            { icon: 'power-outline' as const, color: '#64748b', text: 'Auto-Start After Reboot', interval: 'Always' },
          ].map((item, i) => (
            <View key={i} style={styles.monitoringItem}>
              <Ionicons name={item.icon} size={20} color={item.color} />
              <Text style={styles.monitoringText}>{item.text}</Text>
              <Text style={styles.monitoringInterval}>{item.interval}</Text>
